...
class StrategyConverter:
    ...
    async def parse_natural_language_strategy(self, strategy_text: str) -> QuantifiedStrategy:
        ...
        **重要约束：**
        ...
        - 支持变量赋值: 可以创建中间变量，如 "let MA5 = (close[0] + close[1] + close[2] + close[3] + close[4]) / 5"
        - 支持列表操作和计算
        - 函数必须返回布尔值表示条件是否满足
        - 支持复杂的时序逻辑和统计计算
        
        公式示例：
        - 简单价格比较: "close[-1] > close[-2]"
        - 移动平均计算: "(close[-1] + close[-2] + close[-3] + close[-4] + close[-5]) / 5"
        - 成交量变化率: "(volume[-1] - volume[-2]) / volume[-2] * 100"
        - 涨停数统计: "sum(1 for i in range(60) if pctChg[-i-1] >= 9.9) >= 2 if len(pctChg) >= 60 else False"
        - 价格区间: "high[-1] - low[-1]"
        - 换手率条件: "turn[-1] > 1.0"
        - 价格变化百分比: "pctChg[-1] > 3.0"
        - 复合条件: "close[-1] > preclose and turn[-1] > 2.0"
        - 均线形态: "MA5 = (close[-1] + close[-2] + close[-3] + close[-4] + close[-5]) / 5; MA10 = (close[-1] + close[-2] + ... + close[-10]) / 10; MA5 < MA10 and MA5 > MA10"（实际公式需展开）
        - 涨停潮后回落: "sum(1 for i in range(5) if pctChg[-i-1] >= 9.9) >= 3 if len(pctChg) >= 5 else False and close[-1] < close[-5]"
        - 股价震荡: "(max(close[-i] for i in range(20)) - min(close[-i] for i in range(20))) / min(close[-i] for i in range(20)) < 0.2 if len(close) >= 20 else False"
        - 均线MA5下穿/走平MA10/20: "MA5 = sum(close[-i-1] for i in range(5))/5; MA10 = sum(close[-i-1] for i in range(10))/10; MA20 = sum(close[-i-1] for i in range(20))/20; (MA5 <= MA10 and MA5 <= MA20) or (abs(MA5 - MA10) < MA10*0.001)"
        - 均线MA5上穿MA10但MA20走平: "MA5 = sum(close[-i-1] for i in range(5))/5; MA10 = sum(close[-i-1] for i in range(10))/10; MA20 = sum(close[-i-1] for i in range(20))/20; (MA5 > MA10 and MA5_prev < MA10_prev) and abs(MA20 - MA20_prev) < MA20*0.001"
        - 90%筹码集中度≤20%（简化模拟）: "(max(close[-i] for i in range(60)) - min(close[-i] for i in range(60))) / ((max(close[-i] for i in range(60)) + min(close[-i] for i in range(60)))/2) * 100 <= 20 if len(close) >= 60 else False"
        - 平均成本与当前股价差值在10%内（简化模拟）: "abs(((sum(close[-i] * volume[-i] for i in range(60)) / sum(volume[-i] for i in range(60))) if len(close) >= 60 else 0) - close[-1]) / close[-1] * 100 <= 10"
